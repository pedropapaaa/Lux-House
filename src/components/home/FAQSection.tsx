...
function FAQItem({ q, a, isOpen, onToggle, index, isMobile }: { q: string; a: string; isOpen: boolean; onToggle: () => void; index: number; isMobile: boolean }) {
  const accent = index % 3 === 0 ? '#FF8C2E' : index % 3 === 1 ? '#D4842E' : '#5B9BD5';
    ...
    >
      ...
      <AnimatePresence initial={false}>
        {isOpen && (
          ...
          >
            <p className="text-xs sm:text-sm sm:text-base text-white/40 leading-relaxed pb-4 sm:pb-6 px-4 sm:px-6 sm:px-8 pl-11 sm:pl-14 sm:pl-16">
              {a}
            </p>
          </motion.div>
        )}
...
const FAQSection = memo(function FAQSection() {
  ...
  return (
    <section id="faq" className="py-16 sm:py-24 sm:py-32 lg:py-40 px-4 sm:px-6 relative bg-dark-950">
      ...
      <div className="max-w-3xl mx-auto relative">
        ...
        >
          ...
          <h2 className="text-3xl sm:text-4xl sm:text-5xl lg:text-6xl font-bold text-white uppercase tracking-tight">
            PERGUNTAS{' '}
            <span
              style={{
        ...
        <motion.div
          ...
          style={{
            ...
            boxShadow: isMobile ? 'none' : '0 0 40px rgba(255, 140, 46, 0.1)',
          }}
        >
          {faqs.map((faq, i) => (
            <FAQItem
