...
function FAQItem({ q, a, isOpen, onToggle, index, isMobile }: { q: string; a: string; isOpen: boolean; onToggle: () => void; index: number; isMobile: boolean }) {
  ...
  return (
    <motion.div
      initial={false}
      className="border-b border-ivory/5 last:border-b-0"
    >
      <button
        onClick={onToggle}
      ...
      <AnimatePresence initial={false}>
        {isOpen && (
          ...
          >
            <p className="text-xs sm:text-sm sm:text-base text-ivory/40 leading-relaxed pb-4 sm:pb-6 px-4 sm:px-6 sm:px-8 pl-11 sm:pl-14 sm:pl-16">
              {a}
            </p>
          </motion.div>
        )}
...
const FAQSection = memo(function FAQSection() {
  ...
  return (
    <section id="faq" className="py-16 sm:py-24 sm:py-32 lg:py-40 px-4 sm:px-6 relative bg-dark-950 grain-overlay">
      {/* Neon background effects - hidden on mobile */}
      {!isMobile && (
        <div className="absolute inset-0 pointer-events-none">
          <div className="absolute top-1/4 left-1/4 w-[400px] h-[400px] rounded-full bg-magenta-500/5 blur-[180px]" />
