  ...
  return (
    <motion.nav
      ...
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
        scrolled
          ? 'bg-dark-950/95 backdrop-blur-xl py-3'
          : 'bg-transparent py-5'
      }`}
      style={{
        borderBottom: scrolled ? '1px solid rgba(255, 90, 0, 0.15)' : 'none',
      }}
    >
      <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
        ...
        <Link to="/" className="flex items-center group">
          ...
          >
            <picture>
              ...
              <img
                ...
                style={{ filter: isMobile ? 'none' : 'drop-shadow(0 0 14px rgba(255, 90, 0, 0.6))' }}
              />
            </picture>
          </motion.div>
          <div className="ml-3 leading-tight">
            ...
            <span className="text-[8px] tracking-[0.5em] text-pink-400/50 uppercase block">Casa de Show</span>
          </div>
        </Link>
            ...
            {links.map((l, i) => (
              ...
              >
                ...
                <span
                  ...
                  style={{
                    background: 'linear-gradient(90deg, #FF5A00, #8B5CF6)',
                    boxShadow: isMobile ? 'none' : '0 0 8px rgba(255, 90, 0, 0.5)',
                  }}
                />
              </motion.button>
            ))}
          ...
          {onBuyClick && canBuy ? (
            <motion.button
              ...
              style={{
                ...
                borderRadius: '12px',
                boxShadow: isMobile ? 'none' : '0 0 20px rgba(255, 90, 0, 0.3), inset 0 0 15px rgba(255, 90, 0, 0.05)',
              }}
            >
              <motion.div
        ...
        {open && (
          ...
          >
            <motion.div
              initial={{ y: -20 }}
              animate={{ y: 0 }}
              className="mx-4 mt-2 rounded-2xl p-5 flex flex-col gap-3"
              style={{
                background: 'rgba(9, 9, 9, 0.95)',
                border: '1px solid rgba(255, 90, 0, 0.2)',
                boxShadow: isMobile ? 'none' : '0 0 30px rgba(255, 90, 0, 0.15)',
              }}
            >
              {isHome && links.map((l, i) => (
                <motion.button
                  ...
                  className="text-left text-sm tracking-widest uppercase text-white/50 hover:text-pink-400 transition-colors py-2.5 px-3 rounded-lg hover:bg-white/5"
                >
                  {l.label}
                </motion.button>
              ))}
              ...
              {onBuyClick && canBuy && (
                <motion.button
                  ...
                  className="relative mt-1 px-5 py-3.5 rounded-xl text-white text-sm tracking-widest uppercase font-bold flex items-center justify-center gap-2 overflow-hidden"
                  style={{
                    border: '2px solid #FF5A00',
                    boxShadow: isMobile ? 'none' : '0 0 20px rgba(255, 90, 0, 0.3)',
                  }}
                >
                  <Ticket size={16} />
                  Comprar Ingresso
              ...
              <Link
                to="/meus-ingressos"
                className="text-sm text-white/40 hover:text-pink-400 tracking-wide py-2.5 px-3 rounded-lg hover:bg-white/5 flex items-center gap-2"
              ...
              <Link
                to="/admin"
                className="text-sm text-white/25 tracking-wide py-2.5 px-3 rounded-lg hover:bg-white/5"
                onClick={() => setOpen(false)}
              >
                Admin
              </Link>
