...
import {
  ...
  Settings, Edit3, AlertCircle, Check, Plus,
...
export default function AdminDashboard() {
  ...
  const createLotMutation = useMutation({
    mutationFn: async ({ name, price, total_quantity }: { name: string; price: number; total_quantity: number }) => {
      const sortOrder = (lots.length > 0 ? Math.max(...lots.map((l) => l.sort_order)) : 0) + 1;
      const { error } = await supabase
        .from('lots')
        .insert({ event_id: selectedEventId, name, price, total_quantity, sold_quantity: 0, status: 'closed', sort_order: sortOrder });
      if (error) throw error;
      await logAudit({ action: 'lot_create', entity_type: 'lot', new_values: { name, price, total_quantity, event_id: selectedEventId } });
    },
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['admin-lots'] });
      qc.invalidateQueries({ queryKey: ['lots'] });
    ...
    onError: (err) => {
      setNewLotError(err instanceof Error ? err.message : 'Erro ao criar lote.');
    },
  });

  const handleCreateLot = () => {
    const price = parseFloat(newLotPrice.replace(',', '.'));
    const qty = parseInt(newLotQuantity, 10);
    if (!newLotName.trim()) { setNewLotError('Informe o nome do lote.'); return; }
    if (isNaN(price) || price < 0) { setNewLotError('Informe um preço válido.'); return; }
    if (isNaN(qty) || qty <= 0) { setNewLotError('Informe uma quantidade válida.'); return; }
    createLotMutation.mutate({ name: newLotName.trim(), price, total_quantity: qty });
  };
  ...
  return (
    <AdminLayout title="Início">
      ...
      >
        <div className="flex items-center justify-between p-5 border-b border-white/5">
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={() => { setNewLotOpen(true); setNewLotError(''); }}
              className="flex items-center gap-2 text-xs text-cyan-400/80 hover:text-cyan-400 transition-colors px-4 py-2 border border-cyan-500/20 rounded-lg hover:border-cyan-500/40"
            >
      ...
      <Modal
        open={newLotOpen}
        onClose={() => { setNewLotOpen(false); setNewLotError(''); }}
        title="Criar Novo Lote"
        maxWidth="md"
      >
        <div className="p-6 space-y-5">
          <div className="flex flex-col gap-2">
            <label className="text-[10px] tracking-[0.2em] uppercase text-white/40 font-medium">Nome do lote</label>
            <input
              type="text"
              value={newLotName}
          ...
          <div className="flex flex-col gap-2">
            <label className="text-[10px] tracking-[0.2em] uppercase text-white/40 font-medium">Preço (R$)</label>
            <input
              type="text"
              value={newLotPrice}
          ...
          {newLotError && (
            <div className="p-4 rounded-xl bg-red-500/10 border border-red-500/25 flex items-start gap-3">
              ...
              <p className="text-sm text-red-400">{newLotError}</p>
            </div>
          )}
          <p className="text-xs text-white/30">O lote sera criado como \"Fechado\". Use \"Definir como lote ativo\" para comecar a venda.</p>
          <div className="flex items-center gap-3">
            <Button variant="ghost" onClick={() => { setNewLotOpen(false); setNewLotError(''); }} className="flex-1">Cancelar</Button>
            <Button onClick={handleCreateLot} loading={createLotMutation.isPending} className="flex-1">Criar lote</Button>
          </div>
        </div>
      </Modal>
