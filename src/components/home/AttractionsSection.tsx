...
const AttractionsSection = memo(function AttractionsSection() {
  ...
  const cards = isComingSoon || schedule.length === 0
    ...
    : schedule.map((s, i) => ({
        ...
        image: s.title === 'DJ Rayder' ? 'https://images.pexels.com/photos/5949086/pexels-photo-5949086.jpeg?auto=compress&cs=tinysrgb&w=600&h=400&fit=crop' : event?.photos?.[i % (event.photos.length || 1)] || `/images/files_10828746-2026-07-03T03-41-0${i % 3 + 4}13Z-image.webp`,
    ...
    <section id="atrações" className="py-16 sm:py-24 sm:py-32 lg:py-40 px-4 sm:px-6 relative bg-dark-900 grain-overlay">
      ...
      <div className="max-w-7xl mx-auto relative">
        ...
        >
          ...
          <div className="flex items-center justify-center gap-2 sm:gap-3 text-ivory/40 text-xs sm:text-sm sm:text-base">
            {isComingSoon ? (
              <span className="text-magenta-400 font-bold">{COMING_SOON}</span>
            ) : (
              <>
                <span>{weekday}</span>
                <div className="w-1.5 h-1.5 rounded-full bg-magenta-500/60" />
                <span className="text-magenta-400 font-bold">{day} de {monthName}</span>
                <div className="w-1.5 h-1.5 rounded-full bg-magenta-500/60" />
        ...
        {isLive && event?.live_info && (
          ...
          >
            ...
            {event.live_info.next_attraction && <div className="text-ivory/40 text-xs mt-1">Próxima atração: {event.live_info.next_attraction}</div>}
        ...
        {isLoading ? (
          <div className="flex justify-center py-20">
            <div className="w-8 h-8 rounded-full border-2 border-magenta-500/30 border-t-magenta-400 animate-spin" />
          </div>
        ) : cards.length === 0 ? (
          <motion.div
            initial={{ opacity: 0 }}
          ...
          >
            <Radio size={40} className="text-ivory/15 mx-auto mb-4" />
                ...
                >
                  ...
                  <div className="relative h-48 sm:h-56 sm:h-64 lg:h-80 overflow-hidden">
                    ...
                    >
                      ...
                      >
                        <Clock size={14} style={{ color: a.accent }} className="sm:!size-4" />
                        <span className="text-sm sm:text-lg font-mono font-bold" style={{ color: a.accent }}>{a.time}</span>
                      </div>
                    </motion.div>
                  ...
                  <div className="p-4 sm:p-6 lg:p-8">
                    ...
                    <h3 className="text-lg sm:text-2xl font-display font-bold text-ivory mb-2 sm:mb-3 group-hover:text-magenta-400 transition-colors duration-300">
                      {a.artist}
                    </h3>
        ...
        {isEnded && event?.ended_info && (
          ...
          >
            {event.ended_info.final_message && <p className="text-ivory/70 text-sm mb-4">{event.ended_info.final_message}</p>}
            {event.ended_info.next_event_name && (
              <div className="text-ivory/50 text-sm">
                Próximo evento: <span className="text-magenta-400 font-semibold">{event.ended_info.next_event_name}</span>
                {event.ended_info.next_event_date && <span className="text-ivory/40"> — {event.ended_info.next_event_date}</span>}
              </div>
            )}
          </motion.div>
        )}
